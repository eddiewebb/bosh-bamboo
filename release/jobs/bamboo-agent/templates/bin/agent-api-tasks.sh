...
purgeCapabilities(){
	curl -H "X-Atlassian-Token:nocheck" -X DELETE -k -b \${TMPDIRD}/cookies "\${bambooUrl}/rest/agents/latest/\${agentId}/capabilities?uuid=\${uuid}" -o \$TMPDIRD/state.txt 2>/dev/null
}
...
getStatus(){
	curl -H "X-Atlassian-Token:nocheck" -X GET -k -b \${TMPDIRD}/cookies "\${bambooUrl}/rest/agents/latest/\${agentId}/status/text?uuid=\${uuid}" -o \$TMPDIRD/state.txt 2>/dev/null	
...
disableAgent(){
	curl -H "X-Atlassian-Token:nocheck" -X POST -k -b \${TMPDIRD}/cookies "\${bambooUrl}/rest/agents/latest/\${agentId}/state/disable?uuid=\${uuid}" -o \$TMPDIRD/state.txt 2>/dev/null	
...
disableAgent(){
	curl -H "X-Atlassian-Token:nocheck" -X POST -k -b \${TMPDIRD}/cookies "\${bambooUrl}/rest/agents/latest/\${agentId}/state/enable?uuid=\${uuid}" -o \$TMPDIRD/state.txt 2>/dev/null	
